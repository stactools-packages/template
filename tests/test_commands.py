...
import pystac
from stactools.package.commands import create_package_command
from stactools.testing import CliTestCase


class CreateCollectionTest(CliTestCase):
    def create_subcommand_functions(self):
        return [create_package_command]

    def test_create_collection(self):
        with TemporaryDirectory() as tmp_dir:
            # Run your custom create-collection command and validate
            # Example:
            result = self.run_command(["package", "create-collection"])
    ...
    def test_create_item(self):
        with TemporaryDirectory() as tmp_dir:
            ...
            # Example:
            result = self.run_command(["package", "create-item"])
            self.assertEqual(result.exit_code,
                             0,
                             msg="\n{}".format(result.output))
